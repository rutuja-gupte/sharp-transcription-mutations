...
Process id was 66173 - Warning! This one will not give the right results
...
Running the code again with minor fixes at 69268 - Warning! This is the one that is likely to give the right results. But test the results thoroughly please.
...
This is a placeholder for some text and code for combining the replication data with the overall output.
...
```{r}
test.rv <- rv %>% select(CHROM, POS, REF, ALT, TYPE, RDH, MAT, ploidy, chr_copy_num, indel.type, genic, SGDID, mut.type)
...
test.1.1 <- d.test.1 %>% filter(!is.na(allele.freq.x))
```
