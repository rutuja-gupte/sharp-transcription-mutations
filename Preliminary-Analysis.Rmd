...
For each site in the genome, determining the direction and magnitude of the most likely transcript. Changing the functions to check overlap to work the opposite way to make things faster.

```{r}
row.names(d) <- as.character(d$chr_num*1e8 + d$pos)

d$xu.overlap <- 0
d$vand.overlap <- 0
d$feat.overlap <- 0

for (i in 1:nrow(xu.tr)){
  chr <- xu.tr[i, "chr_num"]
  left <- xu.tr[i, "left"] + chr*1e8
  right <- xu.tr[i, "right"] + chr*1e8
  for (j in left:right){
    d[as.character(j), "xu.overlap"] = d[as.character(j), "xu.overlap"] + 1
  }
}

for (i in 1:nrow(vand.tr)){
  chr <- vand.tr[i, "chr_num"]
  left <- vand.tr[i, "left"]
  right <- vand.tr[i, "right"]
  for (j in left:right){
    vand.overlap[j] = vand.overlap[j] + 1
  }
}
...
for (i in 1:nrow(feat)){
  ...
  right <- feat[i, "right"]
  for (j in left:right){
    feat.overlap[j] = feat.overlap[j] + 1
  }
}


d$xu.overlap <- xu.overlap
d$vand.overlap <- vand.overlap
d$feat.overlap <- feat.overlap
```


This is very inefficient. But so is the one above. Doesnt hurt to try at this point.
