...
```{r}
net.plus<-read.delim("transcription_data/plus_test.txt",h=F,sep=" ")
names(net.plus)<-c("pos","value")
net.minus<-read.delim("transcription_data/minus_test.txt",h=F,sep=" ")
names(net.minus)<-c("pos","value")
```

Trimming the datasets to shorter testing datasets
```{r}
# test.plus <- net.plus[1:100000, ]
# test.minus <- net.plus[1:100000, ]
# 
# rm(net.minus)
# rm(net.plus)
```
...
```{r}
# ggplot() + geom_line(aes(x=test.plus$pos, y=test.plus$value), color='red') +
#   geom_line(aes(test.minus$pos, y=test.minus$value), color='blue')
```

Joining and comparing the datasets to understand the difference
```{r}
# test.diff <- full_join(test.minus, test.plus, by='pos')
# test.diff$diff = test.diff$value.x - test.diff$value.y
# head(test.diff)
# test.diff %>% filter(is.na(value.y))
# test.diff %>% filter(is.na(value.x))
# test.diff %>% filter(diff != 0)
```

Renaming the test datasets to original names for convenience.

```{r}
# net.plus <- test.plus
# net.minus <- test.minus
# rm(test.plus)
# rm(test.minus)
```
...
# net.plus$reads<-net.plus$y/0.4819878
# net.minus$reads<-net.minus$y/0.4819878
# 
# xu.tr<-read.delim("transcription_data/xu_annotations.txt")
# xu.tr$chr_num<-xu.tr$chr
# xu.tr$left<-xu.tr$start
# xu.tr$right<-xu.tr$end
# xu.tr$chr_name<-sapply(xu.tr$chr,function(x){chr_names[x]})
# 
...
# 	if(str=="+"){
# 		k<-net.plus[net.plus$chr_num==chr.i & net.plus$pos>=pos.l & net.plus$pos<=pos.r,]}else{
# 			k<-net.minus[net.minus$chr_num==chr.i & net.minus$pos>=pos.l & net.minus$pos<=pos.r,]
# 		}
# 	total<-sum(k$reads)
# 	density<-total/(pos.r-pos.l)
# 	if(str=="+"){
...
# 	}
# 	if(pos.r-pos.l<700){warn<-1}else{warn<-0}
