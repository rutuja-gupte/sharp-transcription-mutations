...
Processing the replication data as required and storing the processed final data to avoid repetitive computations
...
rep.dynamics$rep.slope<-sapply(1:nrow(rep.dynamics),function(r){
	find.rep.slope(rep.dynamics$chr[r],rep.dynamics$pos[r],loess.models)
})

rep.dynamics$rep.sign <- sign(rep.dynamics$rep.slope)
...
rep.dynamics$rep.fitted<-sapply(1:nrow(rep.dynamics),function(r){
	find.rep.smooth(rep.dynamics$chr[r],rep.dynamics$pos[r],loess.models)
})

rep.dynamics %>% 
  # filter(chr_num == 5) %>% 
  ggplot() + 
  geom_line(aes(x=pos, y=ratio), color='red') +
  geom_line(aes(x=pos, y=rep.fitted), color='blue') +
  facet_wrap(vars(chr_num))
```

Testing this cuz this is being funny

Reading in the features to check with all the ARS
...
```{r}
# ggplot() +
#   geom_line(data=rep.dynamics[rep.dynamics$chr_num==1,], 
#             mapping = aes(x=pos, y=ratio))
...
rep.dynamics %>% 
  ...
  geom_vline(xintercept = 
               feat[(feat$feat_type == "ARS_consensus_sequence") &
                    (feat$chr_num == 10),]$left, color='black')
```

```{r}
muller_ori <- read.csv("muller_rep.csv")

chr_names<-c("chrI","chrII","chrIII","chrIV","chrV","chrVI","chrVII","chrVIII", "chrIX","chrX","chrXI","chrXII","chrXIII","chrXIV","chrXV","chrXVI")

muller_ori$chr_num <- sapply(muller_ori$chr,function(x){
  which(chr_names==x)
  })
...
rep.dynamics %>% 
  ...
  geom_line(aes(x=pos, y=rep.fitted), color='blue') +
  facet_wrap(vars(chr_num)) +
  geom_vline(data = muller_ori,mapping = aes(xintercept=loc)) +
  facet_wrap(vars(chr_num))
```


```{r}
muller_ori <- read.csv("rep.csv")
...
rep.dynamics %>% 
  filter(chr_num %in% c(5,6,7,8)) %>%
  ggplot() + 
  geom_line(aes(x=pos, y=ratio), color='red') +
  geom_line(aes(x=pos, y=rep.fitted), color='blue') +
  facet_wrap(vars(chr_num)) +
  geom_vline(data = muller_ori %>% filter(chr_num %in% c(5,6,7,8)),
             mapping = aes(xintercept=loc), linetype="dashed") +
  facet_wrap(vars(chr_num))
```


```{r}
muller_ori <- read.csv("rep2.csv")
muller_ori$chr_num <- muller_ori$Chromosome
...
rep.dynamics %>% 
  ...
  geom_line(aes(x=pos, y=rep.fitted), color='blue') +
  facet_wrap(vars(chr_num)) +
  geom_vline(data = muller_ori %>% filter(Chromosome %in% c(5,6,7,8)),
             mapping = aes(xintercept=Start), linetype="dashed") +
  facet_wrap(vars(chr_num))
```
...
```{r}
write.csv(rep.dynamics, "replication_data/clean_rep.csv")
```
